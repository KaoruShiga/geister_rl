...
def selfplay(file_name, agent=None):
    ...
    plt_bttl = 200
    linestyles = ['-']  # alphaに相当 # linestyle=(0, (1, 0))
    plt_colors = ['k']  # betaに相当,mマゼンタ(紫),cシアン(青緑)
    linestyle_avg = '-'
    plt_color_avg = 'k'
    alphas = [0.005]
    betas = [0.0003]
    ...
    for episode in range(max_episodes):
        ...
        for i in rnd.sample(range(agents_len), agents_len):  # -> [2, 0, 1]など
            ...
            # for j in rnd.sample(range(agents_len), agents_len):
            agent = agents[i]
            agent.alpha = alphas[i] * (1 - episode/max_episodes)
            agent.beta = betas[i] * (1 - episode/max_episodes)
            agent.learn(env, max_episodes=1)
        # 定期的にランダムとの対戦結果を描画
        if (episode) % plt_intvl == 0:
            episodes_x.append(episode)
    ...
    # selfplay向けに変更
    for i in range(agents_len):
        np.save(file_name+"_w", agents[i].w)
        np.save(file_name+"_theta", agents[i].theta)
...
if __name__ == "__main__":
    import load_
    past_path = "weights/weights_17/vsself3"
    file_name = "weights/weights_17/vsself4"
    game = Geister2()
