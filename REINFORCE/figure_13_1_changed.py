...
class ReinforceAgent:
    ...
    def eligibility_vector(self, x, s):
        return x - self.pi(self.x(s)) * (self.x(s, ShortCorridor.left) + self.x(s, ShortCorridor.right))
...
def experiment(returns, alpha_index, alpha, run_num, random_seed, num_episodes, max_timesteps):
    ...
    for episode in range(num_episodes):
        ...
        while (s is not None) and (t < max_timesteps):
            ...
            x_list.append(x)
            r, ns = ShortCorridor.step(s, a)
            s = ns
            t += 1
            r_list.append(r)
            g += r
        # Store returns:
        returns[alpha_index, run_num, episode] = g
        # learn
        for x, r in zip(x_list, r_list):
            ...
            pis = pis.reshape(2, 1)
            x = x.reshape(2, 1)
            dlt_theta = agent.eligibility_vector(x, s)-(x - xs.dot(pis)).reshape(2, 1)
            assert(np.any(dlt_theta < 1.0e-10))
            agent.theta += agent.alpha*g*(x - xs.dot(pis)).reshape(2, 1)
            g -= r
...
if __name__ == '__main__':
    ...
    random_seeds = [np.random.randint(low=0, high=2**20) for run in range(args.num_runs)]
